  ...
  const filtered = useMemo(() => documents.filter(d => {
    ...
    if (search) {
      ...
      if (!(d.vendor_name ?? "").toLowerCase().includes(q) &&
          !(d.invoice_number ?? "").toLowerCase().includes(q)) return false
    }
    return true
                      ...
                      <Link href={`/documents/${doc.id}/review`} className="flex items-center gap-2.5 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          <div className="text-[11px] text-muted-foreground flex items-center gap-1.5">
                            <span className="truncate max-w-[120px]">{doc.invoice_number ?? doc.id.slice(0, 8)}</span>
                            <span>·</span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filtered.map(doc => (
            <Link
              key={doc.id}
              href={`/documents/${doc.id}/review`}
              className="bg-card border border-border rounded-[12px] p-4 cursor-pointer
                hover:shadow-md hover:border-brand-300 dark:hover:border-brand-600 transition block"
            >
              <div className="aspect-[4/3] rounded-[10px] bg-muted flex items-center justify-center text-5xl mb-3">
                {getThumb(doc.vendor_name)}
              </div>
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-[11px] text-muted-foreground truncate">
                    {doc.invoice_number ?? doc.id.slice(0, 8)}
                  </div>
                </div>
