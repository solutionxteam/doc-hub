...
export async function GET(req: NextRequest) {
  ...
  const { data } = await admin.from("split_bills")
    .select(`id, title, total_amount, category, sport_type, venue, trip_type, destination, split_participants(id, name, is_host:line_user_id)`)
    .eq("share_token", token)
    .single()
  ...
  return NextResponse.json({
    bill: {
      id:           data.id,
      title:        data.title,
      type:         category === "sport" ? "sport" : category === "trip" ? "trip" : "split",
