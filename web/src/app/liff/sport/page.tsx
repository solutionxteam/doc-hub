...
export default function LiffSportDashboard() {
  ...
  async function openDetail(id: string) {
    ...
    setBusy(true)
    try {
      const res = await fetch(`/api/liff/sport-groups/${id}?lineUserId=${profile.userId}`)
      const data = await res.json()
  ...
  async function doAction(action: "join" | "pay" | "unpay" | "finalize") {
    ...
    setBusy(true)
    try {
      const res = await fetch(`/api/liff/sport-groups/${detail.id}`, {
  ...
  async function createGroup() {
    if (!profile || !sportType || !fee) return
    ...
    try {
      const res = await fetch("/api/liff/sport-groups", {
        ...
        body: JSON.stringify({
          ...
          sportType, fee: Number(fee), venue: venue.trim() || undefined,
        }),
      })
      const data = await res.json()
      if (!res.ok) {
        ...
        {view === "create" && (
          <>
            ...
            <div className="bg-card border rounded-2xl p-4 shadow-sm space-y-4">
              ...
              <div>
                <p className="text-sm font-semibold mb-2">สถานที่ (ไม่บังคับ)</p>
                <input
                  value={venue} onChange={e => setVenue(e.target.value)}
                  placeholder="เช่น สนามแบดบางนา"
                  className="w-full h-11 rounded-xl border px-3 text-sm outline-none focus:border-violet-500 focus:ring-2 focus:ring-violet-500/15"
                />
              </div>
              ...
              <button
                onClick={createGroup}
                disabled={!sportType || !fee || Number(fee) <= 0 || busy}
        ...
        {view === "detail" && detail && (
          <>
            ...
            <div className="bg-gradient-to-r from-violet-600 to-indigo-600 rounded-2xl p-4 text-white shadow-lg mb-3">
              {detail.venue && <p className="text-sm text-white/80 flex items-center gap-1 mb-1"><MapPin className="w-3.5 h-3.5" />{detail.venue}</p>}
